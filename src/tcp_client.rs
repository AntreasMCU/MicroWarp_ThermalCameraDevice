...
impl TcpClient{
    pub fn new(addr: SocketAddr) -> Result<TcpClient, Box<dyn Error>>{
        ...
        Ok({
            TcpClient{
                ...
                from_client_rx: Some(from_client_rx),
                from_client_tx: Some(from_client_tx),
                to_client_rx: Some(to_client_rx),
                to_client_tx: Some(to_client_tx),
                stream: None
            }
        })
    }
    ...
    pub fn start(&self) -> Result<(mpsc::Receiver<Vec<u8>>, mpsc::Sender<Vec<u8>>), Box<dyn Error>>{
        ...
        let read_from_client_tx = mpsc::Sender::clone(&self.from_client_tx.unwrap());
        ...
        let shared_receiver = self.from_client_rx.take();
        if shared_receiver.is_none(){
            return Err(Box::new(ioError::new(ErrorKind::Other, "Failed to retrive mpsc retriever from client")));
        }

        let shared_sender = self.from_client_tx.expect("Failed to retrive mpsc sender from client").clone();
